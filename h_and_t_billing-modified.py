...
class HandTBilling(Document):
	...
	def get_all_data_calcalation(self):
		...
		for vender in self.get("h_and_t_table",{"check":1}):
			...
			doc = frappe.db.get_list("Cane Weight",
											filters={"docstatus": 1,"date": ["between", [self.from_date, self.to_date]],"season" : self.season ,"branch" : self.branch ,"h_and_t_billing_status":False},
											fields=["harvester_contract","harvester_code","transporter_code","harvester_name","transporter_name","contract_id","distance","actual_weight","vehicle_type","cart_number"],)
			...
			for d in doc:
				if(str(vender.vender_id)==str(d.harvester_code) and str(vender.type)==str("Harvester")):
					...
					"child_h_and_t_invisible",
					{
						...
						"contract_id":d.harvester_contract,
						"distance":d.distance,
						"distance_amt":round(float(self.get_rate(d.distance,str(d.vehicle_type),"Harvester")),2) if (self.get_rate(d.distance,str(d.vehicle_type),"Harvester")) else frappe.throw(f"Please create the rate chart for vehicle type '{d.vehicle_type}' in Harvester Rate Chart"),
						"weight":round((d.actual_weight),3),
						"total":round((float(d.actual_weight)*float(self.get_rate(d.distance,str(d.vehicle_type),"Harvester"))),2) if (self.get_rate(d.distance,str(d.vehicle_type),"Harvester")) else frappe.throw(f"Please create the rate chart for vehicle type  '{d.vehicle_type}' in Harvester Rate Chart"),
				...
				if(str(vender.vender_id)==str(d.transporter_code)and str(vender.type)==str("Transporter")):
					...
					"child_h_and_t_invisible",
					{
						...
						"contract_id":d.contract_id,	
						"distance":d.distance,
						"distance_amt":round((float(self.get_rate(d.distance,str(d.vehicle_type),"Transporter"))),2) if (self.get_rate(d.distance,str(d.vehicle_type),"Transporter")) else frappe.throw(f"Please create the rate chart for vehicle type  '{d.vehicle_type}' in Transporter Rate Chart"),
						"weight":round((d.actual_weight),3),
						'total':round(d.actual_weight*float(self.get_rate(d.distance,str(d.vehicle_type),"Transporter")),2) if (self.get_rate(d.distance,str(d.vehicle_type),"Transporter")) else frappe.throw(f"Please create the rate chart for vehicle type  '{d.vehicle_type}' in Transporter Rate Chart"),
						"vehicle_type":d.vehicle_type,
						"cartno":d.cart_number,
						"other_id":d.harvester_contract
					}
				)
		# frappe.throw("invisible table")
		...
		for d in data_calculation_dict:
			...
			if self.includes_sales_invoice_deduction: 
				if(data_calculation_dict[d]["type"]=="Transporter"):
					...
					sales_invoices=[{"Sales invoice ID": d_d.name,"Outstanding Amount": d_d.outstanding_amount,"Account": d_d.debit_to,"Contract Id":d_d.h_and_t_contract,"Sale Type":d_d.custom_sale_type}for d_d in deduction_doc]  # in this list all sales invoice will recored with there accound and outstanding_amount info
				# sales_invoice_ded=[]
				...
				if(data_calculation_dict[d]["type"]=="Harvester"):
					...
					# Sales Invoice calculation for Store Material
					deduction_doc_store = frappe.get_all("Sales Invoice",
																	filters={"h_and_t_contract":data_calculation_dict[d]["contract_id"],"status": ["in", ["Unpaid", "Overdue", "Partly Paid"]],"custom_sale_type":"Store Material"},
																	fields=["outstanding_amount", "customer", "name", "debit_to","h_and_t_contract","custom_sale_type"],)

					sales_invoices_store_material=[{"Sales invoice ID": d_s.name,"Outstanding Amount": d_s.outstanding_amount,"Account": d_s.debit_to,"Contract Id":d_s.h_and_t_contract,"Sale Type":d_s.custom_sale_type}for d_s in deduction_doc_store]
				# sales_invoice_store_ded=[]
				# if int(len(contract_dict[str(data_calculation_dict[d]["contract_id"])]))>1:
				# 	for i in range(len(contract_dict[str(data_calculation_dict[d]["contract_id"])])):
				# 		if(str(d)!=str(contract_dict[str(data_calculation_dict[d]["contract_id"])][i])):
				# 			index=str(contract_dict[str(data_calculation_dict[d]["contract_id"])][i])
				...
				# 			parsed_list = ast.literal_eval(formatted_input)
				# 			if(len(parsed_list)>=8):
				# 				sales_invoice_store_ded=parsed_list[8]
				# 				if(sales_invoice_store_ded):
				# 					for j in sales_invoices_store_material:
				# 						for k in sales_invoice_store_ded:
				# 							if(j["Sales invoice ID"] == k["Sales invoice ID"] ):
				# 								j["Outstanding Amount"]=j["Outstanding Amount"]-k["Outstanding Amount"]		
				# 					sales_invoices_store_material = [m for m in sales_invoices_store_material if m["Outstanding Amount"] != 0]
					frappe.msgprint(f"<b>sales_invoices_store_material: </b>{sales_invoices_store_material}")
					sales_invoice_deduction_store_material_amt = sum(float(d["Outstanding Amount"]) for d in sales_invoices_store_material)
					frappe.msgprint(f"sales_invoice_deduction_store_material_amt: {sales_invoice_deduction_store_material_amt}")
			...
			if(payable_amt>=0):
				...
				all_deduction =(loan_installment_intrest + loan_installment + sales_invoices+other_deduction_dict+tds_ded_list_tras+sd_ded_list_tr+hire_cherge_list+penalty_deuction_li+sales_invoices_store_material)  # frappe.msgprint(str(all_deduction))
				...
				while float(total_of_h_t) <= float(total_deduction):
					...
					total_sum = float(sum([
											...
											+ float(entry.get("Store Material Deduction", 0))
				...
				contains_key = next((key for key in ["Outstanding Amount","Installment Amount","Installment Interest Amount","Deduction Amount","Hire Charge Amount","Penalty Amount","Store Material Deduction"] if key in last_poped_entry),None,)
				if (str(contains_key)) == "Outstanding Amount":
					# if last_poped_entry.get('Sale Type') == 'Diesel Sale':
				...
				all_deduction.append(last_poped_entry)
					...
					# for store material deduction
				# if contains_key == "Outstanding Amount":
				# 	if last_poped_entry.get('Sale Type') == 'Store Material':
				# 		store_material_deduction_amount = round(float(total_payable), 2)
				# 		total_deduction = round(float(total_deduction) + float(total_payable), 2)
				# 		total_payable = 0
				# 		# Update the deduction amount for store material
				# 		if 'Store Material Deduction' in last_poped_entry:
				# 			last_poped_entry['Store Material Deduction'] += store_material_deduction_amount
				# 		else:
				# 			last_poped_entry['Store Material Deduction'] = store_material_deduction_amount
				# 	all_deduction.append(last_poped_entry)
				...
				sales_invoice_deduction_amt = sum(float(record['Outstanding Amount']) for record in all_deduction if 'Outstanding Amount' in record and data_calculation_dict[d]["type"]=="Transporter")
				sales_invoice_deduction_store_material_amt = sum(float(record.get('Outstanding Amount', 0)) for record in all_deduction if record.get('Sale Type') == 'Store Material' and data_calculation_dict[d]["type"]=="Harvester")
				...
				sales_invoices_store_material = [record for record in all_deduction if 'Store Material Deduction' in record and record.get('Sale Type') == 'Store Material'] 
			...
			data_calculation_dict[d]["all_deduction_information"]=str(sales_invoices)+str(loan_installment)+str(loan_installment_intrest)+str(other_deduction_dict)+str(tds_ded_list_tras)+str(sd_ded_list_tr)+str(hire_cherge_list)+str(penalty_deuction_li) + str(sales_invoices_store_material)
		...
		for d in data_calculation_dict:
			self.append(
						"calculation_table",	
						{
							...
							"transporter":str(data_calculation_dict[d]["transporter"]),
	...
	def je_of_sales_invoice_and_farmer_loan(self):
				...
				if(s.type=="Transporter" and s.vehicle_type=="BULLOCK CART" and s.transporter==s.vender_id and s.partner_id==s.contract_id):
					...
					for k in self.get("calculation_table"):
						if(k.vehicle_type=="BULLOCK CART" and k.vender_id==s.vender_id and k.contract_id==s.contract_id):
							...
							list_data_hr=parsed_list[6]
							list_data_pen=parsed_list[7]
							list_data_store = parsed_list[8] #mine
							if list_data_se:
							...
							# For sales invoice store material
							if list_data_store:
								for data_store in list_data_store:
		...
		# total_debit = sum(item.get('debit_in_account_currency', 0) or 0 for item in je.accounts)
		# total_credit = sum(item.get('credit_in_account_currency', 0) or 0 for item in je.accounts)

		# frappe.throw(f"Total Debit: {total_debit} === Total Credit: {total_credit}")
